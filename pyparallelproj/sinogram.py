...
class PETSinogram:
  ...
  def __init__(self, scanner, data = None, span = 1, rtrim = 30):
    if span != 1:
      ...
      self.nrad    = scanner.ncrystals_per_plane//2 - 2*self.rtrim
      self.nviews  = scanner.ncrystals_per_plane
  ...
  def get_view_crystal_indices(self, views):
    ...
    for i, view in enumerate(views):
      i_tr_start[:,i] = ((np.arange(self.scanner.ncrystals_per_plane // 2) + view) % 
                              self.scanner.ncrystals_per_plane)[self.rtrim:-self.rtrim]
      i_tr_end[:,i]   = ((-np.arange(1, 1 + self.scanner.ncrystals_per_plane // 2) + view) % 
                              self.scanner.ncrystals_per_plane)[self.rtrim:-self.rtrim]
...
if __name__ == '__main__':
  ...
  scanner = RegularPolygonPETScanner(ncrystals_per_module = np.array([16,9]),
                                     nmodules             = np.array([28,3]))
  sino    = PETSinogram(scanner)

  fig,ax = py.subplots(3,5, figsize = (15,9))
 
  views = np.arange(15)*(448//15)
