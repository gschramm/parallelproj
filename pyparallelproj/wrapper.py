...
def joseph3d_fwd(xstart, xend, img, img_origin, voxsize, img_fwd, nLORs, img_dim, 
  ...
  if n_visible_gpus > 0:
    if isinstance(img, cp.ndarray):
      ok = joseph3d_fwd_cuda_kernel((math.ceil(nLORs/threadsperblock),), (threadsperblock,), 
                                    (xstart.ravel(), xend.ravel(), img.ravel(), cp.asarray(img_origin), 
                                    cp.asarray(voxsize), img_fwd, np.int64(nLORs), cp.asarray(img_dim)))
    else:
...
def joseph3d_back(xstart, xend, back_img, img_origin, voxsize, sino, nLORs, img_dim,
  ...
  if n_visible_gpus > 0:
    if isinstance(sino, cp.ndarray):
      ok = joseph3d_back_cuda_kernel((math.ceil(nLORs/threadsperblock),), (threadsperblock,), 
                                     (xstart.ravel(), xend.ravel(), back_img, cp.asarray(img_origin), 
                                     cp.asarray(voxsize), sino.ravel(), np.int64(nLORs), cp.asarray(img_dim)))
    else:
      nvox = ctypes.c_longlong(img_dim[0]*img_dim[1]*img_dim[2])
...
def joseph3d_fwd_tof_sino(xstart, xend, img, img_origin, voxsize, img_fwd, nLORs, img_dim,
  ...
  if n_visible_gpus > 0:
    if isinstance(img, cp.ndarray):
      ok = joseph3d_fwd_tof_sino_cuda_kernel((math.ceil(nLORs/threadsperblock),), (threadsperblock,), 
                                             (xstart.ravel(), xend.ravel(), img.ravel(), cp.asarray(img_origin), 
                                             cp.asarray(voxsize), img_fwd, np.int64(nLORs), cp.asarray(img_dim),
                                             np.int16(ntofbins), np.float32(tofbin_width), 
                                             cp.asarray(sigma_tof).ravel(), cp.asarray(tofcenter_offset).ravel(), 
                                             np.float32(nsigmas), lor_dependent_sigma_tof, lor_dependent_tofcenter_offset))
    else:
      nvox = ctypes.c_longlong(img_dim[0]*img_dim[1]*img_dim[2])
...
def joseph3d_back_tof_sino(xstart, xend, back_img, img_origin, voxsize, sino, nLORs, img_dim,
  ...
  if n_visible_gpus > 0:
    if isinstance(sino, cp.ndarray):
      ok = joseph3d_back_tof_sino_cuda_kernel((math.ceil(nLORs/threadsperblock),), (threadsperblock,), 
                                              (xstart.ravel(), xend.ravel(), back_img, cp.asarray(img_origin), 
                                              cp.asarray(voxsize), sino.ravel(), 
...
def joseph3d_fwd_tof_lm(xstart, xend, img, img_origin, voxsize, img_fwd, nLORs, img_dim,
  ...
  if n_visible_gpus > 0:
    if isinstance(img, cp.ndarray):
      ok = joseph3d_fwd_tof_lm_cuda_kernel((math.ceil(nLORs/threadsperblock),), (threadsperblock,), 
                                           (xstart.ravel(), xend.ravel(), img.ravel(), cp.asarray(img_origin), 
                                            cp.asarray(voxsize), img_fwd, np.int64(nLORs), 
                                            cp.asarray(img_dim),
...
def joseph3d_back_tof_lm(xstart, xend, back_img, img_origin, voxsize, lst, nLORs, img_dim,
  ...
  if n_visible_gpus > 0:
    if isinstance(lst, cp.ndarray):
      ...
      ok = joseph3d_back_tof_lm_cuda_kernel((math.ceil(nLORs/threadsperblock),), (threadsperblock,), 
                                            (xstart.ravel(), xend.ravel(), back_img, cp.asarray(img_origin), 
                                             cp.asarray(voxsize), lst, np.int64(nLORs), 
                                             cp.asarray(img_dim), np.float32(tofbin_width), cp.asarray(sigma_tof).ravel(), 
                                             cp.asarray(tofcenter_offset).ravel(), np.float32(nsigmas), 
                                             tofbin, lor_dependent_sigma_tof, lor_dependent_tofcenter_offset))
    else:
      nvox = ctypes.c_longlong(img_dim[0]*img_dim[1]*img_dim[2])
