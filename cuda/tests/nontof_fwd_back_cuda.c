...
int main()
{
    ...
    float **d_img = copy_float_array_to_all_devices(img, n0 * n1 * n2);
    ...
    float **d_bimg = copy_float_array_to_all_devices(bimg, n0 * n1 * n2);
    joseph3d_back_cuda(xstart, xend, d_bimg, img_origin, voxsize, ones, nlors, img_dim, tpb);
    sum_float_arrays_on_first_device(d_bimg, n0 * n1 * n2);
    get_float_array_from_device(d_bimg, n0 * n1 * n2, 0, bimg);
