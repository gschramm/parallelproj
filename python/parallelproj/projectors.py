...
def demo_ParallelViewProjector2D(
    img_shape: tuple[int, int],
    xp: ModuleType,
    dev: str,
    num_views: int = 180,
    num_rad: None | int = None,
    radius: None | float = None,
    voxel_size: tuple[float, float] = (1., 1.)
) -> ParallelViewProjector2D:
    """wrapper function to create simple demo 2D parallel view projector
    ...
    img_shape : tuple[int, int]
        the image shape
    xp : ModuleType
        the array module (numpy, cupy, pytorch)
    dev : str
        the device (cpu, cuda)
    num_views : int, optional
        number of view, by default 180
    num_rad : None | int, optional
        number of radial elements, by default None
    radius : None | float, optional
        radius of the "scanner", by default None
    voxel_size : tuple[float, float], optional
        voxel size, by default (1., 1.)
    ...
    """
    if radius is None:
        radius = 0.6 * math.sqrt(img_shape[0]**2 + img_shape[1]**2)

    if num_rad is None:
        num_rad = int(2 * radius + 1)

    radial_positions = xp.linspace(-radius, radius, num_rad, device=dev)

    view_angles = xp.linspace(0,
                              2 * math.pi,
                              num_views,
                              endpoint=False,
                              device=dev)
    ...
    voxel_size = (1., 1.)
    img_origin = (-img_shape[0] / 2 + 0.5, -img_shape[1] / 2 + 0.5)

    proj = ParallelViewProjector2D(img_shape, radial_positions, view_angles,
                                   radius, img_origin, voxel_size)
...
#    import matplotlib.pyplot as plt
#
#    proj = demo_ParallelViewProjector2D((100, 100), xp, dev)
#    x = xp.zeros(proj.in_shape, device=dev)
#    x[10:-10, 20:-20] = 1
#    x += 1
#
#    y = proj(x)
...
#    ax[0].imshow(np.asarray(to_device(x, 'cpu')))
#    ax[1].imshow(np.asarray(to_device(y, 'cpu')))
#    fig.tight_layout()
#    plt.show()