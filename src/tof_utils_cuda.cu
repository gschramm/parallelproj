                                 ...
                                 float *half_erf_lut)
{
  ...
  dtof = sqrtf((x_c0 - x_v0)*(x_c0 - x_v0) + (x_c1 - x_v1)*(x_c1 - x_v1) + (x_c2 - x_v2)*(x_c2 - x_v2));
  dtof_far  = dtof + 0.5*tofbin_width;
  dtof_near = dtof - 0.5*tofbin_width;

  // calculate the TOF weight
  //tw = 0.5*(erff(dtof_far/(sqrtf(2)*sigma_tof)) - erff(dtof_near/(sqrtf(2)*sigma_tof)));

  ilut_near = (3 + (dtof_near/(sqrtf(2)*sigma_tof)))/0.001;
  ilut_far  = (3 + (dtof_far /(sqrtf(2)*sigma_tof)))/0.001;
