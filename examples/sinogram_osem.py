...
def main():
    ...
    scanner = ppp.RegularPolygonPETScanner(ncrystals_per_module=np.array(
        [16, 1]),
    ...
    res_model_data = ppp.ImageBasedResolutionModel(
        fwhm_data, ndimage_module=ndimage_module)
    ...
    # setup the acquisition models for data generation
    res_model = ppp.ImageBasedResolutionModel(fwhm,
    ...
    ax[3].plot(np.arange(1, os_emtv.cost.shape[0] + 1),
               os_emtv.cost,
               label='OS-EMTV')
    ax[3].plot(np.arange(1, spdhg.cost.shape[0] + 1),
               spdhg.cost,
               label='SPDHG')
    ax[3].set_ylim(min(os_emtv.cost.min(), spdhg.cost.min()),
                   os_emtv.cost.max())
