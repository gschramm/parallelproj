...
for i in range(num_iter):
    ...
    if i_ss < len(subset_op_A):
        ...
        # ---
        # ---
        # calculate metrics to get better step size ratio
        tmp1 = (x_old - x) / T - subset_op_A[i_ss].adjoint(subset_y[i_ss] - y_plus) / p
        tmp2 = (
            (subset_y[i_ss] - y_plus) / subset_S_A[i_ss] - subset_op_A[i_ss](x_old - x)
        ) / p

        v = float(xp.sum(xp.abs(tmp1)))
        d = float(xp.sum(xp.abs(tmp2)))

        print(f"{i:03}d {v:.2e} {d:.2e}")
        # ---
        # ---
        ...
        # ---
        # calculate metrics to get better step size ratio
        tmp1 = (x_old - x) / T - op_G.adjoint(w - w_plus) / p
        tmp2 = ((w - w_plus) / S_G - op_G(x_old - x)) / p

        v = float(xp.sum(xp.abs(tmp1)))
        d = float(xp.sum(xp.abs(tmp2)))

        print(f"{i:03}g {v:.2e} {d:.2e}")
        # ---
        # ---
