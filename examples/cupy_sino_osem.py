...
def osem(em_sino, acq_model, niter, verbose=False, xstart=None):
    ...
    for isub in range(proj.nsubsets):
        sens_img[isub,
                 ...] = acq_model.back(xp.ones(proj.subset_sino_shapes[isub],
                                               dtype=xp.float32),
                                       isub=isub)
...
class PETAcqModel:
    ...
    def fwd(self, img, isub=None):
        ...
        if isub is None:
            ...
            sino = self.sens_sino[ss] * self.attn_sino[
    ...
    def back(self, sino, isub=None):
        if isub is None:
            ...
            ss = self.proj.subset_slices[isub]
            back_img = self.proj.back_project_subset(self.sens_sino[ss] *
                                                     self.attn_sino[ss] * sino,
                                                     subset=isub)
