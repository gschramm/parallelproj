...
def power_iterations(x,
    ...
    if precond:
        fwd_ones = ppp.pet_fwd_model(np.ones(x.shape, dtype=np.float32),
                                     proj,
                                     attn_sino,
                                     sens_sino,
                                     fwhm=fwhm)
        ...
        back_ones = ppp.pet_back_model(np.ones(proj.sino_params.shape,
                                               dtype=np.float32),
                                       proj,
                                       attn_sino,
                                       sens_sino,
    ...
    for i in range(npower):
        fwd = sqr_S * ppp.pet_fwd_model(
            x * sqr_T, proj, attn_sino, sens_sino, fwhm=fwhm)
        back = sqr_T * ppp.pet_back_model(
            sqr_S * fwd, proj, attn_sino, sens_sino, fwhm=fwhm)
...
back_ones_no_att = ppp.pet_back_model(np.ones(proj.sino_params.shape,
                                              dtype=np.float32),
                                      proj,
                                      sens_sino,
                                      sens_sino,
...
eigen = power_iterations(test_img,
                         proj,
                         sens_sino,
                         sens_sino,
                         fwhm,
                         npower,
                         precond=False)
eigen2 = power_iterations(test_img,
                          proj,
                          sens_sino,
                          sens_sino,
                          fwhm,
                          npower,
                          precond=True)
eigen3 = power_iterations(test_img,
                          proj,
                          attn_sino,
                          sens_sino,
                          fwhm,
                          npower,
                          precond=False)
eigen4 = power_iterations(test_img,
                          proj,
                          attn_sino,
                          sens_sino,
                          fwhm,
